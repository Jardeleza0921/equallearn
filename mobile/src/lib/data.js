...
const URL_PLACEHOLDERS = new Set(['n/a','na','none','null','undefined','-','—','not available']);
export const normalizeExternalUrlValue = value => {
  const raw = clean(value);
  if (!raw || URL_PLACEHOLDERS.has(raw.toLowerCase())) return '';
...
export const saveModule = async ({ id, uid, title, description, order, status, fileURL = '', fileName = '', filePath = '', fileId = '', fileType = '', fileSize = 0 }) => {
  const payload = { title: clean(title), description: clean(description), order: Number(order) || 1, status: status || 'active', fileURL: normalizeExternalUrlValue(fileURL), fileName: clean(fileName), filePath: clean(filePath), fileId: clean(fileId), fileType: clean(fileType), fileSize: Number(fileSize) || 0, updatedAt: serverTimestamp() };
...
export const saveLesson = async ({ id, uid, moduleId, moduleName, title, description, videoUrl = '', fileUrl = '', fileName = '', filePath = '', fileId = '', fileType = '', fileSize = 0, quizPublished }) => {
  const payload = { moduleId, moduleName: clean(moduleName), title: clean(title), description: clean(description), videoUrl: normalizeExternalUrlValue(videoUrl), fileUrl: normalizeExternalUrlValue(fileUrl), fileName: clean(fileName), filePath: clean(filePath), fileId: clean(fileId), fileType: clean(fileType), fileSize: Number(fileSize) || 0, updatedAt: serverTimestamp() };
  if (typeof quizPublished === 'boolean') payload.quizPublished = quizPublished;
  if (id) { await updateDoc(doc(db, COLLECTIONS.lessons, id), payload); return id; }
  const created = await addDoc(collection(db, COLLECTIONS.lessons), { ...payload, quizPublished: typeof quizPublished === 'boolean' ? quizPublished : false, createdBy: uid, createdAt: serverTimestamp() });
  return created.id;
};
export const deleteLesson = id => deleteDoc(doc(db, COLLECTIONS.lessons, id));
export const setLessonQuizPublished = (id, published, uid = '') => updateDoc(doc(db, COLLECTIONS.lessons, id), {
  quizPublished: Boolean(published),
  quizPublishedAt: published ? serverTimestamp() : null,
  quizPublishedBy: published ? uid : '',
  updatedAt: serverTimestamp(),
});
export const lessonQuizIsPublished = lesson => lesson?.quizPublished !== false;
